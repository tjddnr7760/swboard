...
public interface ChatRepository extends JpaRepository<Chat, Long> {
    ...
    @Query("SELECT new swempire.server.domain.chat.dto.ChatResponse(c.id, c.body, c.section, c.parentChatId, c.sequence, c.member.name, c.createdAt) " +
            "FROM Chat c " +
    ...
    @Query("SELECT c FROM Chat c WHERE c.parentChatId = :parentChatId ORDER BY c.sequence DESC")
